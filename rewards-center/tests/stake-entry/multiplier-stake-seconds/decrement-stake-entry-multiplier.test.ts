...
const stakeEntryMultiplierSeconds = 2;
...
test("Decrement multiplier stake seconds", async () => {
  ...
  expect(stakeEntry.parsed.multiplierStakeSeconds?.toNumber()).toBeLessThan(4);
  expect(
    stakeEntry.parsed.multiplierStakeSeconds?.toNumber()
