...
export const commandName = "setStakeEntryMultiplierStakeSeconds";
export const description =
  "Initialize stake entries and set multiplier stake seconds";
...
export const getArgs = (_connection: Connection, _wallet: Wallet) => ({
  ...
  // multiplier to set
  stakeEntryMultiplierSeconds: 30000,
...
export const handler = async (
  ...
  let entryDatas: { mintId: PublicKey; stakeEntryMultiplierSeconds: number }[] =
    [];
  if (args.entryFile && entryDatas.length === 0) {
    ...
    entryDatas = file.split(",\n").map((v) => ({
      ...
      stakeEntryMultiplierSeconds: args.stakeEntryMultiplierSeconds,
    }));
  }
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    for (let j = 0; j < chunk.length; j++) {
      const { mintId, stakeEntryId, stakeEntryMultiplierSeconds } = chunk[j]!;
      ...
      if (
        ...
        (stakeEntry.type === "stakeEntry" &&
          stakeEntry.parsed.multiplierBasisPoints?.toNumber() !==
            stakeEntryMultiplierSeconds)
      ) {
        const ix = await rewardsCenterProgram(connection, wallet)
          .methods.setStakeEntryMultiplierStakeSeconds(
